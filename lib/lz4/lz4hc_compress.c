 ...
 * For the full license text see lib/lz4/lz4defs.h
 *
...
/*
 * LZ4HC_HEAPMODE :
 * Select how stateless HC compression functions allocate memory for their
 * workspace: in stack (0:fastest), or in heap (1:requires malloc()).
 * The kernel uses the stack here, since it does not provide malloc().
 */
...
/*
 * The optimal parser (levels 10..12 upstream) is disabled in the kernel:
 * it requires a ~64 KB workspace which cannot live on the kernel stack,
 * and the kernel API does not provide an allocation for it.
 */
#define LZ4HC_CLEVEL_MAX	9

#include <linux/limits.h>

/* defined later in this file */
LZ4_streamHC_t *LZ4_initStreamHC(void *buffer, size_t size);
void LZ4_resetStreamHC_fast(LZ4_streamHC_t *streamHCPtr,
			    int compressionLevel);
void LZ4_setCompressionLevel(LZ4_streamHC_t *streamHCPtr,
			     int compressionLevel);
...
static const cParams_t k_clTable[LZ4HC_CLEVEL_MAX+1] = {
    ...
    { lz4hc,    64, 16 },  /* 7 */
    { lz4hc,   128, 16 },  /* 8 */
    { lz4hc,   256, 16 },  /* 9==LZ4HC_CLEVEL_MAX */
};
            ...
            )
{
    ...
    {   cParams_t const cParam = LZ4HC_getCLevelParams(cLevel);
        ...
        if (cParam.strat == lz4mid) {
            ...
            /* LZ4HC_CLEVEL_MAX == 9, so only lz4mid/lz4hc are ever selected */
