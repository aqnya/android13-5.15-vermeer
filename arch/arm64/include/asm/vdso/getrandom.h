...
/*
 * The layout of the vvar pages, which mirrors the enum in
 * asm/vdso/vsyscall.h. It is duplicated here to avoid pulling the kernel-only
 * timekeeping headers into the vDSO.
 */
enum vvar_pages {
...
static __always_inline const struct vdso_rng_data *__arch_get_vdso_rng_data(void)
{
	...
	if (IS_ENABLED(CONFIG_TIME_NS) && _vdso_data->clock_mode == VDSO_CLOCKMODE_TIMENS)
		return (void *)&_vdso_rng_data + VVAR_TIMENS_PAGE_OFFSET * (1UL << CONFIG_ARM64_PAGE_SHIFT);
