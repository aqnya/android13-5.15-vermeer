...
/*
 * Kept for ABI compatibility only: the GKI module sched-walt.ko imports this
 * exported symbol.  EEVDF uses sysctl_sched_base_slice instead.
 */
...
found:
	...
	/*
	 * Defensive: pick_next_task_fair() dereferences the result and
	 * group_cfs_rq(se) faults on NULL.  Never return NULL while there is
	 * an entity in the tree to run.
	 */
	if (!best)
...
/*
 * Re-weight the entity while preserving lag and the relative virtual
 * deadline.
 *
 * VRUNTIME
 * --------
 *
 * COROLLARY #1: The virtual runtime of the entity needs to be adjusted if
 * re-weight at !0-lag point.
 *
 * COROLLARY #2: Re-weight does NOT affect weighted average vruntime of all
 * the entities.
 *
 * So the effect of re-weight on vruntime should be:
 *
 *	v' = V' - (V - v) * w / w' = V - vl * w / w' = V - vl'
 *
 * DEADLINE
 * --------
 *
 * When the weight changes, the virtual time slope changes and we should
 * adjust the relative virtual deadline accordingly:
 *
 *	d' = V + (d - V)*w/w'
 */
...
place_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	trace_android_rvh_place_entity(cfs_rq, se,
			(flags & ENQUEUE_INITIAL) ? 1 : 0, &vruntime);
}
...
static void check_preempt_wakeup(struct rq *rq, struct task_struct *p, int wake_flags)
{
	...
	trace_android_rvh_check_preempt_wakeup(rq, p, &preempt, &ignore,
			wake_flags, se, pse, next_buddy_marked, sysctl_sched_base_slice);
	if (preempt)
		goto preempt;
	if (ignore)
		return;
